...
网盘提取密码：${p}
资源展示页：
`+f,r=`upload/${t}/${n}/userinfo.txt`,l=`upload/${t}/${n}/status.json`,c={title:e,status:"审核中",section:u,note:"",link:""},d=(await client.put(r,new Blob([i],{type:"text/plain"})),await client.put(l,new Blob([JSON.stringify(c)],{type:"application/json"})),document.getElementById("filePicker").files);for(let e=0;e<d.length;e++)await client.put(`upload/${t}/${n}/files/`+d[e].name,d[e]);submitted+=1,await client.put(`upload/${t}/submitted.json`,new Blob([JSON.stringify(submitted)],{type:"application/json"})),window.location.href="/submissionsuccess/"}catch(e){console.error("上传失败:",e),alert("投稿失败，请重试！")}}else alert("请填写内容的标题。")}else alert("您还未登录，请登录后再进行投稿。")}()},document.getElementById("SaveDraft").addEventListener("click",async()=>{if(s){if(confirm("确认保存草稿吗？如您之前有草稿内容，此操作会覆盖前一次的草稿内容。")){var e=document.getElementById("input").value,t=`upload/${curemail}/${submitted+1}/draft.json`,e=new Blob([JSON.stringify({content:e},null,2)],{type:"application/json"});try{await client.put(t,e),alert("草稿已保存！")}catch(e){console.error("保存草稿时出错:",e),alert("保存草稿失败，请稍后再试。")}}}else alert("非法操作！请先登录。")}),document.getElementById("LoadDraft").addEventListener("click",async()=>{if(s){var e=`https://download.xn--xhq44jb2fzpc.com/upload/${curemail}/${submitted+1}/draft.json`;try{var t=await fetchNoCache(e);if(t.ok){var n=await t.json();confirm("此操作会覆盖您当前的输入内容，确认加载草稿吗？")&&(document.getElementById("input").value=n.content,preview.innerHTML=marked.parse(n.content),alert("草稿已加载！"))}else{if(404!==t.status)throw new Error("无法加载草稿内容");alert("没有草稿记录！")}}catch(e){console.error("加载草稿时出错:",e),alert("加载草稿失败，请稍后再试。")}}else alert("非法操作！请先登录。")});